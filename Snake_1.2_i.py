...
def load_photo(width = 0, height = 0, url = "", background = False):
    ...
    photo_image = Image.open(BytesIO(photo_image_data))
    if background:
...
root = tk.Tk()
root.title("Snake V 1.1 by Artur Otto")
pygame.init()
...
##################################### ФОТО PLAY ####################################################
play_image = Image.open("C:\\Users\\Asus\\Pictures\\snake_game_photo\\purple buttons\\start_1.png")
play_new_width = 200
play_new_height = 75
play_image = play_image.resize((play_new_width, play_new_height))
play_image_to_game = ImageTk.PhotoImage(play_image)
##################################### ФОТО EXIT ####################################################
exit_image = Image.open("C:\\Users\\Asus\\Pictures\\snake_game_photo\\purple buttons\\start_2.png")
exit_new_width = 100
exit_new_height = 50
...
# backgroung_menu_response = requests.get('https://raw.githubusercontent.com/teenee0/Snake_game/main/background_menu.jpg')
# backgroung_menu_image_data = backgroung_menu_response.content
#
# background_menu_image = Image.open(BytesIO(backgroung_menu_image_data))
...
##################################### ФОТО HARD GAME MODE ####################################################
hard_game_mode_image = Image.open("hard_game_mode.png")
hard_game_mode_new_width = 50
hard_game_mode_new_height = 40
...
# def load_photo(width = 100, height = 100, url = "", background = False):
#     photo_response = requests.get(
#         url)
#     photo_image_data = photo_response.content
#
...
def start_menu(event):
    ...
    canvas.pack()
    play_button = canvas.create_image(canvas_width/2, canvas_height/2-100, image=play_image_to_game, activeimage = exit_image_to_game )
    exit_button = canvas.create_image(canvas_width/2, canvas_height/2, image=exit_image_to_game)
